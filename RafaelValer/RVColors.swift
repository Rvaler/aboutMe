//
//  RVColors.swift
//  RafaelValer
//
//  Created by Rafael Valer on 4/22/15.
//  Copyright (c) 2015 RafaelValer. All rights reserved.
//

import Foundation
import UIKit

class RVColors {
    
    let grayColor = UIColor(red: 246/255, green: 246/255, blue: 246/255, alpha: 1.0)
    let lightBlueColor = UIColor(red: 102/255, green: 171/255, blue: 1, alpha: 1.0)
    let lightOrangeColor = UIColor(red: 1, green: 171/255, blue: 102/255, alpha: 1.0)
    let lightGreenColor = UIColor(red: 113/255, green: 187/255, blue: 64/255, alpha: 1.0)
    let lightPurpleColor = UIColor(red: 210/255, green: 72/255, blue: 122/255, alpha: 1.0)
    let blueColor = UIColor(red: 64/255, green: 157/255, blue: 187/255, alpha: 1.0)
    
    let blueCellColor = UIColor(red: 102/255, green: 171/255, blue: 1, alpha: 0.05)
    let greenCellColor = UIColor(red: 113/255, green: 187/255, blue: 64/255, alpha: 0.05)
    let orangeCellColor = UIColor(red: 1, green: 171/255, blue: 102/255, alpha: 0.05)
    let purpleCellColor = UIColor(red: 210/255, green: 72/255, blue: 122/255, alpha: 0.05)
    let mediumBlueCellColor = UIColor(red: 64/255, green: 157/255, blue: 187/255, alpha: 0.05)
    
}